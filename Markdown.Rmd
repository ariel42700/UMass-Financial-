...
```{r}
x.test <- x[,5:4]
x.test <- as.data.frame(t(x.test))

xt.2 <- lm(V1~V2,data=x.test)
summary(x.2)

xt.3 <- lm(V1~V3,data=x.test)
summary(x.3)

xt.4 <- lm(V1~V4,data=x.test)
summary(x.4)

xt.5 <- lm(V1~V5,data=x.test)
summary(x.5)

xt.6 <- lm(V1~V6,data=x.test)
summary(x.6)

xt.7 <- lm(V1~V7,data=x.test)
summary(x.7)
...
```{r}
res.v6t = pred.v5 - test$V12
res.v11t = pred.v11 - test$V12
res.v3t = predict.v3 - test$V12
res.x2t = predict.x2 - test$V12
res.x7t = predict.x7 - test$V12

sum(res.v6t) #-3.49246e-10
sum(res.v11t)# 0
sum(res.v3t)# 3.49246e-10
sum(res.x2t)# -356311 # the last two are really bad
sum(res.x7t) #124829573
```
```{r}
predt.v11 <- predict(fit.11, test)
predt.v5 <- predict(fit.5, test)
predictt.v3 <- predict(fit.3, test)
predictt.x2 <- predict(x.2, test)
predictt.x7 <- predict(x.7, test)

test.ts <- ts((as.numeric(as.vector(test$V12))), frequency = 1, start = c(2015, 1))

v11t.ts <- ts(as.numeric(pred.v11), frequency = 1, start = c(2015, 1))
v5t.ts <- ts(as.numeric(pred.v5), frequency = 1, start = c(2015, 1))
v3t.ts <- ts(as.numeric(predict.v3), frequency = 1, start = c(2015, 1))
x2t.ts <- ts(as.numeric(predict.x2), frequency = 1, start = c(2015, 1))
x7t.ts <- ts(as.numeric(predict.x7), frequency = 1, start = c(2015, 1))
